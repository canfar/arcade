...
with open('../script/list_of_ms.txt') as fp:
	for line in fp:
		...
		#Pull out the calibrators
		print 'Splitting calibrator sources'
		calibFields = msmd.fieldsforintent("CALIB*",True)
		ncalib = len(calibFields)	#number of unique calibration fields
		#print calibFields
		#loop through each calibrator source and pull out the information
		#HK note to self: unlike IDL, using a range of 0 to N-1 misses the last result
		for i in range(0,ncalib):	
			...
			os.system('rm -rf '+line_sho+'.'+calibFields[i]+'.ms.split.cal')
			#split out the calibrator
			split(vis=line,outputvis=line_sho+'.'+calibFields[i]+'.ms.split.cal', field=calibFields[i],datacolumn='data')
			#keep track of the target
			cal_obsnames.append(line_sho)
			cal_targetnames.append(calibFields[i])


		#Pull out science targets (same method as for the calibrators)
		print 'Splitting science targets'
		scienceFields = msmd.fieldsforintent("OBSERVE_TARGET*",True)
		nscience = len(scienceFields)
		for i in range(0,nscience):
			print scienceFields[i]
			#eliminate the file if it already exists
			os.system('rm -rf '+line_sho+'.'+scienceFields[i]+'.ms.split.cal')
			#split out the science target
			split(vis=line,outputvis=line_sho+'.'+scienceFields[i]+'.ms.split.cal', field=scienceFields[i],datacolumn='data')
			#keep track of the targets
			sci_obsnames.append(line_sho)
			sci_targetnames.append(scienceFields[i])
